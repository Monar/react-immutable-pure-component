...
describe('Test check', () => {
  describe('should not update', () => {
    it('against empty checklist', () => {
      ...
      expect(result_props).toBe(false);
    });
  ...
  describe('should update', () => {
    it('when prev and next have different items', () => {
      expect(check(undefined, {}, { a: 1 }, 'test')).toBe(true);
      expect(check(undefined, { a: 1 }, {}, 'test')).toBe(true);
      expect(check(undefined, { a: 1 }, { b: 1 }, 'test')).toBe(true);
      expect(check(undefined, { a: 1, c: 1 }, { b: 1 }, 'test')).toBe(true);
      expect(check(undefined, { a: 1 }, { b: 1, c: 1 }, 'test')).toBe(true);
    });
    ...
    it('when modified any of the values', () => {
      ...
      expect(check(undefined, { a: {} }, { a: {} }, 'test')).toBe(true);
  ...
  describe('should work with config', () => {
    it('when empty and values change', () => {
      expect(check([], {}, { a: 1 }, 'test')).toBe(false);
      expect(check([], { a: 1 }, {}, 'test')).toBe(false);
      expect(check([], { a: 1 }, { b: 1 }, 'test')).toBe(false);
      expect(check([], { a: 2 }, { a: 1 }, 'test')).toBe(false);
    });

    it('when defined but other values change', () => {
      expect(check(['a'], {}, {}, 'test')).toBe(false);
      expect(check(['a'], {}, { b: 1 }, 'test')).toBe(false);
      expect(check(['a'], { b: 1 }, {}, 'test')).toBe(false);
      expect(check(['a'], { a: 1, b: 2 }, { a: 1, b: 1 }, 'test')).toBe(false);
      expect(check(['a'], { a: 1 }, { a: 1 }, 'test')).toBe(false);
    });

    it('when one of defined value changes', () => {
      expect(check(['a', 'z'], {}, { a: 1 }, 'test')).toBe(true);
      expect(check(['a', 'z'], { a: 1 }, {}, 'test')).toBe(true);
  ...
  describe('should work with config Iterable items in config', () => {
    it("when value under path don't change", () => {
      let path = ['a', 'b'];
      let prev = { a: { b: 10 }, x: 0 };
      expect(check([path], prev, { ...prev, x: 10 }, 'test')).toBe(false);
      expect(check([path], prev, { a: prev.a }, 'test')).toBe(false);
      expect(
        check(
          [path],
          { i: fromJS(prev) },
    ...
    it('when value under path does not exists', () => {
      let path = ['a', 'b'];
      let prev = { a: {}, x: 0 };
      expect(check([path], prev, prev, 'test')).toBe(false);
      expect(check([path], prev, {}, 'test')).toBe(false);
    });
