  ...
  _createClass(ImmutablePureComponent, [{
    ...
    value: function shouldComponentUpdate(nextProps) {
      ...
      function checkProps(name) {
        return immutable.is(immutable.getIn(nextProps, name), immutable.getIn(props, name));
      }

      function checkStates(name) {
        return immutable.is(immutable.getIn(nextState, name), immutable.getIn(state, name));
      }

      var onProps = this.updateOnProps || Object.keys(_objectSpread({}, nextProps, this.props));
      var onState = this.updateOnStates || Object.keys(_objectSpread({}, nextState, state));
      return !onProps.map(guardArray).every(checkProps) || !onState.map(guardArray).every(checkStates);
    }
  }]);
